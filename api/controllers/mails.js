			...
			if (err){
				...
				if(doc){
					...
					sections.forEach(function(s) {
						var left = new Array()
						var right = new Array()
						s.objects = new Array(left,right)
						s.text = '';
...
exports.get_subscribers= function(req, res) {
    User.find({newsletter: true, trust : 'confirmed'}, function(err, users) {
        //var userMap = {}
...
exports.unsubscribe_view = function(req, res) {
       res.send('welcome')
}
    ...
    user.save(function(err) {
        if (err) {
			...
			var user_email = new Object({
				...
				'bodytext':'Veuillez cliquer ce <a href="http://localhost:8087/api/v1/subscribe_confirm?mail='+user.email+'&key='+user.secret+'">lien</a> pour activer votre abonnement à la newsletter à hacktuel</p><p>ou entrez directement cette adresse dans votre navigateur : http://localhost:8087/api/v1/subscribe_confirm?mail='+user.email+'&key='+user.secret+'</p>', 
				'to':user.email
			})
...
exports.subscribe_confirm = function(req, res) {
	

	if(req.query.mail && req.query.key){

		User.findOne({email: req.query.mail})
        .exec(function(err, user) {
            if (err){
            	 res.send(err)
            } 
            if(user){
 				if(user.secret == req.query.key){
...
exports.unsubscribe_post = function(req, res) {
      res.send('welcome')
}
