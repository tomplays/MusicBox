...
$scope.toggle_side_editor = function(obj, zindex) {
	...
	$scope.slide_render_layout= function () {
		...
		if($scope.current_renders_index == _.size($scope.render_available)){
			$scope.current_renders_index=0;
		}

		if($scope.current_renders_index == _.size($scope.render_available)-1){
			$scope.render_layout_next = $scope.render_available[0];
		}
		else{
			$scope.render_layout_next = $scope.render_available[$scope.current_renders_index+1];
		}
		$scope.render_layout = $scope.render_available[$scope.current_renders_index];
